...
const ChatMessage = ({ sender, message, time, isUser, type }: ChatMessageProps & { type: string }) => {
  return (
    <div
      className={`flex flex-col p-4 text-gray-700 rounded-lg m-2 ${
        isUser ? 'bg-green-100 self-end' : 'bg-gray-100 self-start'
      }`}
    >
      <p className="font-semibold text-sm self-start underline pb-2">{isUser ? 'You' : sender}</p>
      {type === 'text' ? (
        <p className=" self-start pt-1 text-gray-700 text-normal">{message}</p>
      ) : type === 'image' ? (
        <img src={message} alt="Sent Image" className="max-w-[200px] max-h-[200px] rounded-lg self-start" />
      ) : type === 'video' ? (
  ...
  return (
    <section className="relative">
      <div className="overflow-y-auto flex flex-col border-[2px] border-gray-300 w-full h-[600px] rounded-md">
        {chats &&
          chats.map((chat, index) => {
            ...
            switch(chat.type){
              case "notify":
                return <p key={index} className="p-4 text-gray-500 rounded-lg m-2 bg-gray-100 self-center text-center">{chat.message}</p>;
              default:
                return <ChatMessage
                key={index}
      ...
      <div className="flex w-full mt-4 gap-4 items-center">
        ...
        />
        <button onClick={handleClick} className='border border-black px-4 py-2 rounded-md'>
        <svg
